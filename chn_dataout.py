...
#            for item in turn.split():
                ...
                if item in eng_noun or item in chn_noun: #ref
                    noun_flag = 1
            if (trial["person"][turnno]==p1):
                prev_item[p1] = prev_item[p1] + jieba.lcut_for_search(turn)
            else:
                prev_item[p2] = prev_item[p2] + jieba.lcut_for_search(turn)
            for bi in get_bigram(jieba.lcut_for_search(turn)):
                if (trial["person"][turnno]==p1):
                    ...
                    if bi in prev_bi[p1]: #alignment
                        ...
                        print(p1+"   "+bi)
            if (trial["person"][turnno]==p1):
                prev_bi[p1] = prev_bi[p1] + get_bigram(jieba.lcut_for_search(turn))
            else:
                prev_bi[p2] = prev_bi[p2] + get_bigram(jieba.lcut_for_search(turn))
